...
<div class="panel panel-flat">
	<div class="panel-heading">
		<h5 class="panel-title">قائمة القاده</h5>
		...
		<tbody>
			@foreach($users as $u)
				<tr id="row-{{$u->id}}">
                    ...
                    <td>{{( (setting('ambassador_num_orders') - $u->num_done_orders) <= 0 )? '0' : (setting('ambassador_num_orders') - $u->num_done_orders)  }}</td>
                    <td>{{setting('ambassador_balance')}}</td>
					<td>{{date('Y-m-d H:i',strtotime($u->userMeta->created_at))}}</td>
					<td>{{date('Y-m-d H:i',strtotime($u->created_at))}}</td>
					<td>{{date('Y-m-d',strtotime($u->created_at.' +'.setting('ambassador_num_days').'days'))}}</td>
					<td>
					<ul class="icons-list">
						<li class="dropdown">
