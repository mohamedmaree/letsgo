...
<div class="panel panel-flat">
	...
	<div class="panel-body">
		<div class="row">
			...
			<div class="col-xs-2">
				<a href="{{route('downloadAllUsers')}}"><button class="btn btn-block btn-float btn-float-lg correspondent" style="background-color:#1b926c; color:#fff;" type="button" ><i class="fa fa-file-excel-o"></i> <span>تحميل Excel </span></button></a>
			</div>	
			<div class="col-xs-2">
		...
		<tbody>
			@foreach($users as $u)
				<tr id="row-{{$u->id}}">
					...
					<td>{{$u->name}}   </td>
					<td>0{{$u->phone}}</td>
					@if(is_null($u->role) || ($u->role == 0) )
					<td>عضو</td>
					@else
					<td>{{($u->Role)? $u->Role->role:''}}</td>
					@endif
                    ...
                    <td>{{($u->captain=='true')?'قائد':'عميل'}}</td>
	        ...
	        <div class="row">
	        	<form action="{{route('adduser')}}" method="POST" enctype="multipart/form-data">
	        		...
	        		<div class="row">
		        		...
		        		<div class="col-sm-9">
		        			<input type="text" name="name" class="form-control" placeholder="الاسم " style="margin-bottom: 10px" required="required"/>
		        			<div class="col-sm-8">
			        ...
			        <div class="col-sm-12" style="margin-top: 10px">
				      	<button type="submit" class="btn btn-primary addCategory" onclick="$(this.form).validate({name: {required: true},{phone: {required: true},{password: {required: true}};
this.form.submit(); this.disabled=true;">اضافه</button>
					...
					<div class="row">
		        		...
		        		<div class="col-sm-6">
							<select name="edit_role" class="form-control" id="permissions">
								<option value="0" > الصلاحية </option>
								@foreach($roles as $role)
									<option value="{{$role->id}}">{{$role->role}}</option>
								@endforeach
							</select>
					...
					<div class="tabbable">
						...
						<div class="tab-content">
							...
							<div class="tab-pane active" id="colored-rounded-justified-tab2">
							    <div class="row">
							    	<form action="{{route('SmsMessageAll')}}" method="POST" enctype="multipart/form-data">
							    		{{csrf_field()}}
							    		<div class="col-sm-12">
							...
							<div class="tab-pane" id="colored-rounded-justified-tab3">
							    <div class="row">
							    	<form action="{{route('notificationAllUsers')}}" method="POST" enctype="multipart/form-data">
							    		{{csrf_field()}}
							    		<div class="col-sm-12">
