	...
	<div class="col-md-12">
		<div class="panel panel-flat">
			...
			<div class="panel-body">
				<div class="row">
					<div class="col-xs-3">
						<a href="{{route('downloadPaymentsReport')}}"><button class="btn btn-block btn-float btn-float-lg correspondent" style="background-color:#1b926c; color:#fff;" type="button" ><i class="glyphicon glyphicon-download"></i> <span>تحميل Excel </span></button></a>
					</div>				
				</div>
			</div>
			<div class="panel-body">
				<div class="tabbable">
					<ul class="nav nav-tabs">
						<!-- supervisors reports  -->
						<li class="active"><a href="#basic-tab1" data-toggle="tab">التعاملات المالية</a></li>
						<!-- users reports -->
										...
										<tr/>
											...
											<td>
												<span class="text-semibold">
							                        @if($payment->operation == 'balance_transfer')
							                            {{ trans('user.balance_transfer') }}
							                        @elseif($payment->operation == 'order_cancel')
							                            {{ trans('user.order_cancel') }}
							                        @elseif($payment->operation == 'order_withdraw')
							                            {{ trans('user.order_withdraw') }}
							                        @elseif($payment->operation == 'order_price')
							                            {{  trans('user.order_price') }}
							                        @elseif($payment->operation == 'guarantee'){
							                            {{  trans('user.guarantee') }}
							                        @elseif($payment->operation == 'reward'){
							                            {{  trans('user.reward') }}
							                        @endif
												</span>
											</td>											
											...
											<td>
												<span class="text-semibold">{{($payment->type == 'add')?'أضافة':'خصم'}}</span>
											</td>
											<td>
