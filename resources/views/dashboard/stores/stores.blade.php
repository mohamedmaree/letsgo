...
@section('content')
	<div class="panel panel-flat">
		<div class="panel-heading">
			<h5 class="panel-title">قائمة المتاجر</h5>
		...
		<div class="panel-body">
			<div class="row">
				<div class="col-xs-3">
					<button class="btn bg-blue btn-block btn-float btn-float-lg openAddModal" type="button" data-toggle="modal" data-target="#exampleModal"><i class="icon-plus3"></i> <span>اضافة متجر</span></button>
				</div>
				<div class="col-xs-3">
					<button class="btn bg-purple-300 btn-block btn-float btn-float-lg" type="button"><i class="icon-list-numbered"></i> <span>عدد المتاجر : {{count($stores)}} </span> </button>
				</div>
			</div>
		</div>
		<!-- /buttons -->
		<table class="table datatable-basic">
			...
			@foreach($stores as $store)
				<tr>
				<!-- <td><img src="{{asset('img/store/icons/'.$store->icon)}}" class="img-circle" alt=""></td> -->
					<td>{{$store->name_ar}}</td>
					<td>{{$store->name_en}}</td>
					<td>{{$store->phone}}</td>
					<td>{{$store->address}}</td>
					<td>{{($store->num_rating > 0)? round(floatval($store->rating / $store->num_rating),1) : '0'}}</td>
					<td>{{App\Stores::where('parent_id',$store->id)->count()}}</td>
					<td>{{Carbon\Carbon::parse($store->created_at)->format('d/m/Y - H:i A')}}</td>
					<td>
						<ul class="icons-list">
							<li class="dropdown">
								...
								<ul class="dropdown-menu dropdown-menu-right">
									...
									<li>
										<a href="#" data-toggle="modal" data-target="#exampleModal2" class="openEditmodal"
										   data-id="{{$store->id}}"
										   data-namear="{{$store->name_ar}}"
										   data-nameen="{{$store->name_en}}"
										   data-phone="{{$store->phone}}"
										   data-email="{{$store->email}}"
										   data-icon="{{$store->icon}}"
										   data-cover="{{$store->cover}}"
										   data-address="{{$store->address}}"
										   data-lat="{{$store->lat}}"
										   data-long="{{$store->lng}}"
										   data-website="{{$store->website}}"
										   data-openfrom='{{$store->open_from}}'							        
								           data-opento='{{$store->open_to}}'
										>
									...
									<!-- branches-->
									<li>
										<a href="{{url('admin/branches/'.$store->id)}}" >
											<i class="glyphicon glyphicon-home"></i>الفروع
										</a>
									</li>
									<!-- menucategories-->
									<li>
										<a href="{{url('admin/menuCategories/'.$store->id)}}" >
											<i class="glyphicon glyphicon-home"></i>أقسام المنيو
										</a>
									</li>
									<!-- products-->
									<li>
										<a href="{{url('admin/products/'.$store->id)}}" >
											<i class="glyphicon glyphicon-home"></i>المنتجات
										</a>
									</li>
									<!-- delete button -->
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalLabel">أضافة متجر جديد</h5>
					</div>
						...
						<div class="row">
							<form action="{{route('createStore')}}" method="POST"  enctype="multipart/form-data" >
								...
								<div class="row">
									...
									<div class="col-sm-8">
										<i class="icon-camera"  onclick="addChooseCover()" style="cursor: pointer;"></i>
										<div class="images-upload-block">
											<input type="file" name="cover" class="image-uploader" id="hidden" >
										</div>
									</div>
									<div class="col-sm-4">
									...
									<div class="col-sm-8">
										<input type="text" name="name_ar" class="form-control" placeholder="اسم المتجر بالعربية " required/>
									</div>
									<div class="col-sm-4">
										<label>الاسم بالانجليزية</label>
									</div>
									<div class="col-sm-8">
										<input type="text" name="name_en" class="form-control" placeholder="اسم المتجر بالانجليزية " required/>
									</div>
									<div class="col-sm-4">
										<label>الهاتف</label>
									</div>
		...
		<div class="modal fade" id="exampleModal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="exampleModalLabel"> تعديل بيانات المتجر </h5>
					</div>
					<div class="modal-body">
							...
							<div class="row">
								<div class="col-sm-12">
									...
									<div class="col-sm-8">
										...
										<input type="file" name="edit_icon" style="display: none;">
									</div>
									<div class="col-sm-4">
										<label>الغلاف</label>
									</div>
									<div class="col-sm-8">
										<img src="" class="cover" style="width: 200px;height: 150;cursor: pointer;margin-bottom:10px;" onclick="ChooseCover()">
										<input type="file" name="edit_cover" style="display: none;">
									</div>
									<div class="col-sm-4">
										<label>الاسم بالعربية</label>
									</div>
									<div class="col-sm-8">
										<input type="text" name="edit_name_ar" class="form-control" placeholder="اسم المتجر بالعربية " required/>
									</div>
									<div class="col-sm-4">
										<label>الاسم بالانجليزية</label>
									</div>
									<div class="col-sm-8">
										<input type="text" name="edit_name_en" class="form-control" placeholder="اسم المتجر بالانجليزية " required/>
									</div>
...
<script type="text/javascript">
	...
	$('.openEditmodal').on('click',function(){
		...
		var link = "{{asset('img/store/icons')}}" +'/'+ icon;
		$(".photo").attr('src',link);

		var coverlink = "{{asset('img/store/cover')}}" +'/'+ cover;
		$(".cover").attr('src',coverlink);
...
<script type="text/javascript">
	...
	function ChooseCover(){$("input[name='edit_cover']").click()}
	function addChooseCover(){$("input[name='cover']").click()}
</script>
@endsection